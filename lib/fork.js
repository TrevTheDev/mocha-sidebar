    ...
    err => {
      if (err.code === 'ENOENT') {
        vscode.window.showErrorMessage(
          'Cannot find Node.js installation from environment variable'
        );
      } else {
...
function nodeJSPath() {
  return new Promise((resolve, reject) => {
    ...
    const searchPaths = paths.reduce(
      (a, p) => a.concat(pathExts.map(ext => path.resolve(p, 'node' + ext))),
      []
    );

    Promise.all(
      searchPaths.map(p =>
        access(p, fs.X_OK).then(
          () => p,
          () => false
        )
      )
