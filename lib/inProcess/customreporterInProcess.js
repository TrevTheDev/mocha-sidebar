...
function Reporter(runner) {
  ...
  runner
    .on('suite', suite => suitePath.push(suite.fullTitle()))
    .on('suite end', () => suitePath.pop())
    .on('pass', test => passed.push(toJS(suitePath, test)))
    .on('fail', test => failed.push(toJS(suitePath, test)))
    .on('end', () => console.error(JSON.stringify({ passed, failed }, null, 2)));
}
